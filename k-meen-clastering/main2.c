...
#define WIDTH   (800/DIVISION)
#define HEIGHT  (600/DIVISION)
...
int main(void){
    ...
    for (size_t i = 0; i < NCLUSTERS; i++)
    {
        ColorOfCluster[i].r = (rand());
        ColorOfCluster[i].g = (rand());
        ColorOfCluster[i].b = rand();
        ColorOfCluster[i].a = rand();
        centoids[i].h = 20;
        centoids[i].w = 20;
    ...
    for (size_t y = 0; y < HEIGHT; y++)
        ...
        int r = rand()%30 + 5,cl = rand()%10+20;

       Distribution(IsToRender,cl,20);
